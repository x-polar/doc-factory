...
브랜드(클라이언트/제품/사내)마다 하위 폴더 1개. 각 문서는 `doc.yaml`의 `brand:`로
어떤 브랜드를 쓸지 고른다. 색·폰트가 다른 문서를 한 공장에서 찍기 위한 구조.
...
## 문서가 브랜드를 고르는 법 (`docs/<문서>/doc.yaml`)
...
2. `doc.yaml`의 `theme:` 블록을 deep-merge로 덮어씀
