...
## notes
T1 드릴다운 (search.hybrid 경로). AO → Query Preprocessing(정규화·확장, DD 참조) → 팬아웃: Term Extraction(text→terms)→Lexical Retriever(BM25) ∥ Query Embedding(MG 경유 text→vector)→Semantic Retriever(top-k)
