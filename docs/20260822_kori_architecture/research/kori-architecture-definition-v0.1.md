...
| D39 | Document Search Internals 드릴다운 슬라이드 신설: T1 내부를 Query Preprocessing(정규화·확장·검색어 분리, DD 참조) → 팬아웃(Keyword Extraction→Lexical Retriever ∥ Query Embedding→Semantic Retriever, 2단×2단 대칭) → Fusion & Rank(RRF)로 상세화. Query Preprocessing·Query Embedding을 T1 내부 스테이지로 명명(임베딩 호출은 Model Gateway 경유). 3장에는 Web Search 고스트 툴 추가(tools are extensible) |
