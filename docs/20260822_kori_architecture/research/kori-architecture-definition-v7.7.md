# KORI Answers 아키텍처 — 구성 요소·관계 정의 v7.7

다이어그램 작도의 기준 문서. v7.2: KORI Answers는 제품 전체 이름으로 확정. v7.3: R15는 다이어그램 미표기. v7.4: 시각화 이원화 및 아이콘 스타일 토큰 시스템 추가. v7.5: C3.1을 C14에 병합, C14를 Deterministic Security Layer로 개명. v7.6: 마스킹/DLP를 설계 범위에서 제외. v7.7: C9 이원 구현(Operational RDB + Analytical DB) 명시, C17 Domain Dictionary 신설(R21), 교체 가능(swappable) 배지 표기 도입.
...
| C9 | Structured Store | 데이터 저장소 | 정형 데이터 + 권한 + 파일 메타(문서 ID, 물리 경로, 버전, 소유자). 논리적 단일 역할, 물리 구현 이원: **Operational RDB**(권한·파일 메타·트랜잭션 정형) + **Analytical DB**(대용량 집계, 예: ClickHouse). C7.3의 SQL 템플릿이 쿼리 성격에 따라 대상 엔진 선택 — 엔진 라우팅은 C7.3 내부 구현으로 다이어그램 미표기(R15와 동일 원칙) |
...
| C17 | Domain Dictionary | 지원 컴포넌트 | 도메인 용어·동의어·엔티티의 단일 원천. 소비처에 참조 제공: C7 쿼리 전처리(동의어 확장·용어 정규화), C15 청킹/색인 보조(용어 인식). 사전 데이터는 고객사별 교체 |
...
| ID | 방향 | 레이블 |
|---|---|---|
| R21 | C17 ⇢ C7·C15 | Domain Term Reference — 쿼리 전처리(C7)·청킹/색인 보조(C15). 참조 관계(점선), 화살표 노드로 그리지 않음 |
...
- **저장소·게이트웨이 레이블**: 역할명을 주 레이블로, 구체 기술 로고를 병기 — 로고는 아이콘이 아니라 전면 하단 고정 배지. C9는 로고 배지 2개(Operational RDB + Analytical DB)
- **교체 가능(swappable) 배지**: 교체 가능 컴포넌트(C5·C6·C9·C10·C11·C12·C16·C17)는 전면 하단에 swappable 마커 병기, 다이어그램 하단 한 줄 범례로 마커 의미 설명. 사이드바 목록은 두지 않음
- **C17**: 바닥 평면의 독립 원기둥(사전 = 데이터) + 소비처(C7·C15)로 얇은 점선. C14 밴드와 시각적으로 구분
...
| D18 | C9는 논리적 단일 Structured Store 유지, 물리 구현 이원(Operational RDB + Analytical DB) — 워크로드(OLTP/OLAP) 차이는 역할 분리 사유가 아님. 엔진 라우팅은 C7.3 내부 구현, 다이어그램 미표기 |
| D19 | C17 Domain Dictionary 신설 — cross-cutting 지원 컴포넌트, C7·C15에 참조 제공(R21, 점선). 사전 데이터는 고객사별 교체 |
| D20 | 교체 가능 컴포넌트(C5·C6·C9·C10·C11·C12·C16·C17)는 swappable 배지 + 하단 범례로 표기 — 별도 사이드바 없음 |
