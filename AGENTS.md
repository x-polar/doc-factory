...
- 상세 작성·디자인 표준은 `reference/style-guide.md`, 색·폰트 값은
  `brand/theme.yaml`, 공유 사실·메시지는 `knowledge/`를 따른다.

## 5. 도구 스택 · 공유 리소스

도구:
- `.pptx` 생성: **python-pptx** (공유 빌더 `src/lib/deckbuilder.py`가 storyboard +
  `brand/theme.yaml`을 읽어 자동 생성)
...
- `brand/` — 디자인 토큰(`theme.yaml`)과 브랜드 가이드. 색·폰트의 **단일 진실원천**.
- `templates/` — 마스터 pptx(브랜드 확정 시). 없으면 빌더가 토큰으로 스타일링.
- `assets/` — 폰트·로고·아이콘 등 바이너리 자산.
...
| `brand/theme.yaml` 토큰 키 | `deckbuilder.py`가 참조하는 키, `reference/chart-style.md` |
...
├── brand/               # 디자인 토큰(theme.yaml) + 브랜드 가이드
...
> 실제 브랜드 자산(마스터 pptx·폰트·로고)은 브랜드 확정 시 `templates/`·`assets/`에
> 채웁니다. 현재 `brand/theme.yaml`은 중립 기본값이며, 빌더는 마스터가 없어도
> 토큰만으로 덱을 생성합니다.
