...
- `.pptx` 생성: **python-pptx** (공유 빌더 `src/lib/deckbuilder.py`가 storyboard +
  문서 `doc.yaml`이 고른 브랜드 테마를 읽어 자동 생성)
...
- `brands/` — **브랜드 레지스트리**. 브랜드마다 1폴더(`theme.yaml`·`brand-guide.md`·
  `assets/`·`templates/`). 문서는 `doc.yaml`의 `brand:`로 선택, `theme:`로 오버라이드.
...
| `doc.yaml` 필드(brand/theme/version) | `deckbuilder.py` 해석 로직, `_template/doc.yaml`, `doc-scaffold` SKILL |
