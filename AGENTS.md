...
- 슬라이드 규격: **브랜드가 결정**(`theme.yaml > slide`). 임의 가정 금지.
  - `xenoimpact` = **A4 인쇄 프리셋 10.83×7.5in** (원본 pptx가 `sldSz type="A4"`).
    화면 전용이 아니라 **인쇄 전제** 문서다.
  - `kori-answers` = 16:9 (웹/화면 아이덴티티).
- **이미지 해상도:** 인쇄용(A4) 브랜드의 배경·도판은 **300dpi 이상**을 유지한다
  (A4 폭 10.83in → 최소 3250px). 화면만 보고 축소하면 인쇄에서 뭉갠다.
...
- **한 슬라이드, 한 메시지.** 제목이 결론을 말하도록 작성(action title).
  - 단, **카탈로그형 문서(회사소개서·제품 브로슈어)는 짧은 라벨 제목**(`Services`,
    `AI Leadership`)을 쓰고 메시지는 `lede`로 내린다. 설득형 문서(제안서·기획서)만
    action title을 쓴다. 원본을 재현하는 문서는 원본의 제목 방식을 따른다.
  - `kicker`가 `title`과 같은 말을 반복하지 않게 한다(중복이면 `kicker`를 뺀다).
...
- **대용량 바이너리는 Git LFS**: `*.pptx`·`*.pdf`는 `.gitattributes`로 LFS 추적됩니다
  (원본 소개서·확정본 등). clone 전 `git lfs install` 필수(미설치 시 포인터만 받음).
  설정·이유는 README "시작하기" 참고.
...
├── .gitattributes       # Git LFS 추적 규칙 (*.pptx, *.pdf)
├── .ai/
